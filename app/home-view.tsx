...
export default function Home({projects}:{projects:ProjectEntry[]}){
 ...
 <section id="work" className="section work"><div className="section-top"><div><p className="eyebrow">01 / {t.nav[0]}</p><h2>{t.selected}<span className="accent">.</span></h2><p>{t.selectedSub}</p></div><div className="filters">{([['all',t.all],['web',t.web],['mobile',t.mobile]] as const).map(([id,label])=><button key={id} aria-pressed={filter===id} className={filter===id?'active':''} onClick={()=>setFilter(id)}>{label}</button>)}</div></div><div className="projects">{visibleProjects.map(({entry,index:i})=>{const cover=entry.content.media.find((media)=>media.type==='image'); return <article className={`project p${i}`} key={entry.slug}><button className={`project-visual${cover?' has-cover':''}`} onClick={e=>openProject(i,e.currentTarget)} aria-label={`${t.details}: ${entry.content.title[locale]}`}>{cover&&<img className="project-cover" src={cover.url} alt="" aria-hidden="true" loading="lazy"/>}<span className="project-number">{entry.number}</span><div className="project-symbol">{entry.symbol}</div><span className="visual-word">{entry.word}</span><span className="project-arrow">↗</span><small>{t.artifact}</small></button><div className="project-info"><p className="eyebrow">{t.tags[i]}</p><button className="project-title" onClick={e=>openProject(i,e.currentTarget)}>{entry.content.title[locale]} <span>↗</span></button><p>{entry.content.shortSummary[locale]}</p></div></article>})}</div></section>
