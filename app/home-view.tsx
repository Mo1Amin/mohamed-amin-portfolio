...
import {slugs,type Category,type ProjectEntry} from './projects-data';
import {MotionRoot,Parallax,Reveal,motion} from './motion';
const anchors=['work','capabilities','about','contact'];
const tabKeys=['ArrowRight','ArrowLeft','ArrowUp','ArrowDown','Home','End'];
export default function Home({projects}:{projects:ProjectEntry[]}){
 ...
 useEffect(()=>{const sync=()=>{const slug=new URLSearchParams(location.search).get('project');setProject(slug&&slugs.includes(slug)?slugs.indexOf(slug):null)};sync();window.addEventListener('popstate',sync);return()=>window.removeEventListener('popstate',sync)},[]);
 ...
 return <MotionRoot><><a className="skip" href="#main">{t.skip}</a><header><a className="brand" href="#" aria-label="Mohamed Amin">ma<span>↗</span></a><nav>{t.nav.map((n,i)=><a key={n} href={`#${anchors[i]}`}>{n}</a>)}</nav><div className="controls"><select aria-label={t.language} value={locale} onChange={e=>setLocale(e.target.value as typeof locale)}><option value="en">EN</option><option value="ar">عربي</option><option value="sv">SV</option></select><button aria-label={t.theme} onClick={()=>setTheme(theme==='dark'?'light':'dark')}>{theme==='dark'?'☼':'◐'}</button><button className="mode" aria-label={t.mode} onClick={()=>setMode(mode==='bold'?'calm':'bold')}>{mode==='bold'?t.bold:t.calm}<span> ◇</span></button></div></header>
 <main id="main"><section className="hero"><motion.div className="hero-copy" initial={{opacity:0,y:42}} animate={{opacity:1,y:0}} transition={{duration:.85,ease:[.22,1,.36,1]}}><p className="eyebrow"><span className="dot"/>{t.eyebrow}</p><h1>{t.head[0]}<br/><span>{t.head[1]}</span><br/>{t.head[2]}<br/><em>{t.head[3]}</em></h1><p className="intro">{t.intro}</p><div className="actions"><a className="primary" href="#work">{t.explore}<span>↗</span></a><a className="text-link" href="https://github.com/Mo1Amin" target="_blank" rel="noreferrer">{t.github} ↗</a></div></motion.div><Parallax className="hero-art"><div aria-hidden="true"><div className="art-grid"/><div className="orbit orbit-one"/><div className="orbit orbit-two"/><div className="orbit orbit-three"/><div className="core"><span>m<span className="core-accent">a</span></span></div><span className="node node-web">&lt;/&gt;<small>WEB</small></span><span className="node node-ai">✳<small>AI / ML</small></span><span className="node node-mobile">▯<small>MOBILE</small></span><span className="coordinate">SYSTEM / AMIN<br/>30.00° N · BUILDING IDEAS</span><span className="art-caption">{t.note}</span><div className="art-index">01 — 03</div></div></Parallax><div className="hero-bottom"><span>{t.scroll} ↓</span><span>WEB / MOBILE / AI</span></div></section>
 <Reveal className="motion-marquee"><div className="marquee" aria-hidden="true"><span>CREATIVE THINKING</span><b>✳</b><span>ENGINEERING MINDSET</span><b>✳</b><span>HUMAN EXPERIENCE</span><b>✳</b></div></Reveal>
 <section id="work" className="section work"><div className="section-top"><div><p className="eyebrow">01 / {t.nav[0]}</p><h2>{t.selected}<span className="accent">.</span></h2><p>{t.selectedSub}</p></div><div className="filters">{([['all',t.all],['web',t.web],['mobile',t.mobile]] as const).map(([id,label])=><button key={id} aria-pressed={filter===id} className={filter===id?'active':''} onClick={()=>setFilter(id)}>{label}</button>)}</div></div><div className="projects">{visibleProjects.map(({entry,index:i})=>{const cover=entry.content.media.find((media)=>media.type==='image'); return <motion.article initial={{opacity:0,y:50,clipPath:'inset(14% 0 0 0)'}} whileInView={{opacity:1,y:0,clipPath:'inset(0% 0 0 0)'}} viewport={{once:true,amount:.12}} transition={{duration:.72,delay:i*.08,ease:[.22,1,.36,1]}} whileHover={{y:-8,rotateX:1.5,rotateY:i%2? -1.5:1.5}} whileTap={{scale:.985}} className={`project p${i}`} key={entry.slug}><motion.button whileTap={{scale:.98}} className={`project-visual${cover?' has-cover':''}`} onClick={e=>openProject(i,e.currentTarget)} aria-label={`${t.details}: ${entry.content.title[locale]}`}>{cover&&<motion.img className="project-cover" src={cover.url} alt="" aria-hidden="true" loading="lazy" whileHover={{scale:1.07}}/>}<span className="project-number">{entry.number}</span><div className="project-symbol">{entry.symbol}</div><span className="visual-word">{entry.word}</span><span className="project-arrow">↗</span><small>{t.artifact}</small></motion.button><div className="project-info"><p className="eyebrow">{t.tags[i]}</p><button className="project-title" onClick={e=>openProject(i,e.currentTarget)}>{entry.content.title[locale]} <span>↗</span></button><p>{entry.content.shortSummary[locale]}</p></div></motion.article>})}</div></section>
