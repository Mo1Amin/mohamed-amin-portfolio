...
import {projectEntries,slugs,type Category} from './projects-data';
...
export default function Home(){
 ...
 const visibleProjects=projectEntries.map((entry,index)=>({entry,index})).filter(({entry})=>filter==='all'||entry.categories.includes(filter));
 ...
 <section id="work" className="section work"><div className="section-top"><div><p className="eyebrow">01 / {t.nav[0]}</p><h2>{t.selected}<span className="accent">.</span></h2><p>{t.selectedSub}</p></div><div className="filters">{([['all',t.all],['web',t.web],['mobile',t.mobile]] as const).map(([id,label])=><button key={id} aria-pressed={filter===id} className={filter===id?'active':''} onClick={()=>setFilter(id)}>{label}</button>)}</div></div><div className="projects">{visibleProjects.map(({entry,index:i})=><article className={`project p${i}`} key={entry.slug}><button className="project-visual" onClick={e=>openProject(i,e.currentTarget)} aria-label={`${t.details}: ${t.projects[i]}`}><span className="project-number">{entry.number}</span><div className="project-symbol">{entry.symbol}</div><span className="visual-word">{entry.word}</span><span className="project-arrow">↗</span><small>{t.artifact}</small></button><div className="project-info"><p className="eyebrow">{t.tags[i]}</p><button className="project-title" onClick={e=>openProject(i,e.currentTarget)}>{t.projects[i]} <span>↗</span></button><p>{t.summaries[i]}</p></div></article>)}</div></section>
 {project!==null&&<section className="case-study section" id="project" aria-labelledby="case-title"><button className="text-link" onClick={()=>openProject(null)}>← {t.back}</button><p className="eyebrow">{t.tags[project]}</p><h2 id="case-title" ref={caseRef} tabIndex={-1}>{t.projects[project]}</h2><p>{t.summaries[project]}</p><h3>{t.role}</h3><p>{t.roles[project]}</p><p className="case-status">{t.release}</p><Link className="primary" href={`/projects/${slugs[project]}`}>{t.view}<span>↗</span></Link></section>}
