...
export default function Home(){
 ...
 {project!==null&&<section className="case-study section" id="project" aria-labelledby="case-title"><button className="text-link" onClick={()=>openProject(null)}>← {t.back}</button><p className="eyebrow">{t.tags[project]}</p><h2 id="case-title" ref={caseRef} tabIndex={-1}>{projectEntries[project].content.title[locale]}</h2><p>{projectEntries[project].content.shortSummary[locale]}</p><h3>{t.role}</h3><p>{projectEntries[project].content.role[locale]}</p><p className="case-status">{t.release}</p><Link className="primary" href={`/projects/${slugs[project]}`}>{t.view}<span>↗</span></Link></section>}
